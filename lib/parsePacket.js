...
function parsePacket(packet) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // also skip corrupted lines (containing the replacement character �)
        if (lines[i].replace(/(\r\n|\n|\r)/gm,"").trim() != "" && !_containsReplacementCharacter(lines[i])) {
            ...
            switch (line.obisCode) {
                ...
                case "1-0:31.7.0":
                    parsedPacket.electricity.instantaneous.current.L1.reading = parseFloat(line.value);
                    parsedPacket.electricity.instantaneous.current.L1.unit = line.unit;
                    break;

                case "1-0:51.7.0":
                    parsedPacket.electricity.instantaneous.current.L2.reading = parseFloat(line.value);
                    parsedPacket.electricity.instantaneous.current.L2.unit = line.unit;
                    break;

                case "1-0:71.7.0":
                    parsedPacket.electricity.instantaneous.current.L3.reading = parseFloat(line.value);
 ...
 *
 * @param str : a line of the P1 packet
 */
function _containsReplacementCharacter(str) {
    return str.includes('\uFFFD');
}
